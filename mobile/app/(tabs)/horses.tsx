...
export default function HorsesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (!user) {
      setHorses([]);
      setError('Please sign in to view your horses.');
      setLoading(false);
      return;
    }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <FlatList
        ...
        renderItem={({ item }) => <HorseCard row={item} onPress={() => router.push(`/(tabs)/horse/${text(item, 'id', 'uuid')}`)} />}
      />
    </SafeAreaView>
  );
}

function HorseCard({ row, onPress }: { row: HorseRow; onPress: () => void }) {
