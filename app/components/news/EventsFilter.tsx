...
type Event = { name: string; href: string; date: string | null; location: string | null };
...
function ExternalIcon() { return <svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M14 5h5v5M19 5l-8 8"/><path strokeLinecap="round" strokeLinejoin="round" d="M19 14v3a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h3"/></svg>; }
...
export default function EventsFilter() {
  ...
  const filtered = useMemo(() => events.filter((event) => {
    ...
    return text.includes(query.trim().toLowerCase()) && (country === "All" || event.location === country) && (month === "All" || (event.date ?? "").startsWith(month));
  }), [events, country, month, query]);
