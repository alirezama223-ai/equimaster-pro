...
type LocaleCopy = {
  ...
  descriptionLabel: string; placeholder: string; empty: string; pending: string; done: string; cancelled: string;
  types: Record<string, string>; leads: Record<string, string>; required: string; error: string;
};

const copy: Record<string, LocaleCopy> = {
  en: { title: "Reminders", subtitle: "Keep training, health and care dates in one place.", add: "Add reminder", cancel: "Cancel", complete: "Mark complete", create: "Create reminder", close: "Close", titleLabel: "Title", typeLabel: "Type", horseLabel: "Horse", noHorse: "General reminder", dateLabel: "Date & time", leadLabel: "Remind me before", descriptionLabel: "Notes", placeholder: "e.g. Annual vaccination", empty: "No reminders yet.", pending: "Pending", done: "Completed", cancelled: "Cancelled", required: "Please enter a title and date.", error: "Something went wrong.", types: { training: "Training", vaccination: "Vaccination", dental: "Dental", farrier: "Farrier", medication: "Medication", vet: "Vet", custom: "Custom" }, leads: { "0": "At the time", "5": "5 minutes", "10": "10 minutes", "30": "30 minutes", "60": "1 hour", "1440": "1 day", "2880": "2 days" } },
  de: { title: "Erinnerungen", subtitle: "Training, Gesundheit und Pflege an einem Ort verwalten.", add: "Erinnerung hinzufügen", cancel: "Abbrechen", complete: "Als erledigt markieren", create: "Erinnerung erstellen", close: "Schließen", titleLabel: "Titel", typeLabel: "Typ", horseLabel: "Pferd", noHorse: "Allgemeine Erinnerung", dateLabel: "Datum & Uhrzeit", leadLabel: "Vorher erinnern", descriptionLabel: "Notizen", placeholder: "z. B. Jährliche Impfung", empty: "Noch keine Erinnerungen.", pending: "Offen", done: "Erledigt", cancelled: "Abgebrochen", required: "Bitte Titel und Datum eingeben.", error: "Etwas ist schiefgelaufen.", types: { training: "Training", vaccination: "Impfung", dental: "Zähne", farrier: "Hufschmied", medication: "Medikament", vet: "Tierarzt", custom: "Eigene" }, leads: { "0": "Zum Zeitpunkt", "5": "5 Minuten", "10": "10 Minuten", "30": "30 Minuten", "60": "1 Stunde", "1440": "1 Tag", "2880": "2 Tage" } },
  fr: { title: "Rappels", subtitle: "Gardez les dates d'entraînement, de santé et de soins au même endroit.", add: "Ajouter un rappel", cancel: "Annuler", complete: "Marquer comme terminé", create: "Créer le rappel", close: "Fermer", titleLabel: "Titre", typeLabel: "Type", horseLabel: "Cheval", noHorse: "Rappel général", dateLabel: "Date et heure", leadLabel: "Me prévenir avant", descriptionLabel: "Notes", placeholder: "ex. Vaccination annuelle", empty: "Aucun rappel pour le moment.", pending: "En attente", done: "Terminé", cancelled: "Annulé", required: "Veuillez saisir un titre et une date.", error: "Une erreur est survenue.", types: { training: "Entraînement", vaccination: "Vaccination", dental: "Dentaire", farrier: "Maréchal-ferrant", medication: "Médicament", vet: "Vétérinaire", custom: "Personnalisé" }, leads: { "0": "À l'heure", "5": "5 minutes", "10": "10 minutes", "30": "30 minutes", "60": "1 heure", "1440": "1 jour", "2880": "2 jours" } },
  nl: { title: "Herinneringen", subtitle: "Beheer trainings-, gezondheids- en verzorgingsdata op één plek.", add: "Herinnering toevoegen", cancel: "Annuleren", complete: "Als voltooid markeren", create: "Herinnering maken", close: "Sluiten", titleLabel: "Titel", typeLabel: "Type", horseLabel: "Paard", noHorse: "Algemene herinnering", dateLabel: "Datum en tijd", leadLabel: "Herinner mij vooraf", descriptionLabel: "Notities", placeholder: "bijv. jaarlijkse vaccinatie", empty: "Nog geen herinneringen.", pending: "Open", done: "Voltooid", cancelled: "Geannuleerd", required: "Vul een titel en datum in.", error: "Er ging iets mis.", types: { training: "Training", vaccination: "Vaccinatie", dental: "Gebit", farrier: "Hoefsmid", medication: "Medicatie", vet: "Dierenarts", custom: "Aangepast" }, leads: { "0": "Op tijdstip", "5": "5 minuten", "10": "10 minuten", "30": "30 minuten", "60": "1 uur", "1440": "1 dag", "2880": "2 dagen" } },
  es: { title: "Recordatorios", subtitle: "Gestiona las fechas de entrenamiento, salud y cuidados en un solo lugar.", add: "Añadir recordatorio", cancel: "Cancelar", complete: "Marcar como completado", create: "Crear recordatorio", close: "Cerrar", titleLabel: "Título", typeLabel: "Tipo", horseLabel: "Caballo", noHorse: "Recordatorio general", dateLabel: "Fecha y hora", leadLabel: "Avisarme antes", descriptionLabel: "Notas", placeholder: "p. ej. vacunación anual", empty: "Aún no hay recordatorios.", pending: "Pendiente", done: "Completado", cancelled: "Cancelado", required: "Introduce un título y una fecha.", error: "Algo salió mal.", types: { training: "Entrenamiento", vaccination: "Vacunación", dental: "Dental", farrier: "Herrador", medication: "Medicación", vet: "Veterinario", custom: "Personalizado" }, leads: { "0": "A la hora", "5": "5 minutos", "10": "10 minutos", "30": "30 minutos", "60": "1 hora", "1440": "1 día", "2880": "2 días" } },
};
...
export default function RemindersManager({ reminders, horses }: Props) {
  ...
  const [form, setForm] = useState({ title: "", description: "", reminderType: "training", horseId: "", dueAt: "", remindBeforeMinutes: 10 });
  ...
  async function submit(event: React.FormEvent) {
    ...
    setForm({ title: "", description: "", reminderType: "training", horseId: "", dueAt: "", remindBeforeMinutes: 10 });
      ...
      <div className="mt-6 space-y-3">
        ...
        {reminders.map((reminder) => (
          <article key={reminder.id} className="rounded-2xl border border-white/10 bg-[#0B1422] p-4 sm:flex sm:items-center sm:justify-between sm:gap-5">
            <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-white">{reminder.title}</h3><span className="rounded-full border border-white/10 px-2 py-1 text-[11px] text-gray-400">{t.types[reminder.reminder_type] ?? reminder.reminder_type}</span></div><p className="mt-2 text-sm text-gray-300">{formatDate(reminder.due_at)}{reminder.remind_before_minutes ? ` · ${t.leads[String(reminder.remind_before_minutes)] ?? `${reminder.remind_before_minutes} min`}` : ""}</p>{reminder.description && <p className="mt-1 text-sm text-gray-500">{reminder.description}</p>}</div>
            {reminder.status === "pending" && reminder.enabled ? <div className="mt-4 flex shrink-0 gap-2 sm:mt-0"><button disabled={busy} onClick={() => action(reminder.id, "complete")} className="rounded-lg border border-emerald-500/20 px-3 py-2 text-xs font-semibold text-emerald-300">{t.complete}</button><button disabled={busy} onClick={() => action(reminder.id, "cancel")} className="rounded-lg border border-white/10 px-3 py-2 text-xs font-semibold text-gray-400">{t.cancel}</button></div> : <span className="mt-3 inline-block text-xs text-gray-500 sm:mt-0">{reminder.status === "completed" ? t.done : t.cancelled}</span>}
          </article>
        ))}
      </div>
    </section>
