    ...
    <footer className="border-t border-white/10 bg-[#07101f] text-gray-400">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          <Link href="/" className="text-lg font-black tracking-tight text-white">SHABDIZ</Link>
          <p className="mt-1 max-w-full text-sm">{l.tagline}</p>
        </div>
        <nav aria-label="Footer" className="flex flex-wrap gap-x-6 gap-y-3 pr-14 text-sm md:pr-52">
          <Link href="/about" className="transition hover:text-white">{l.about}</Link>
          <Link href="/marketplace" className="transition hover:text-white">{l.marketplace}</Link>
          <Link href="/legal" className="font-medium text-gray-300 transition hover:text-white">{l.legal}</Link>
        </nav>
      </div>
    </footer>
  );
