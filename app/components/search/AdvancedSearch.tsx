...
export default function AdvancedSearch({
  ...
  const countryOptions = COUNTRIES.map((countryOption) => {
    const label = localizeSearchOptionLabel(countryOption.name, countryOption.name, locale);
    return {
      value: countryOption.name,
      label,
      searchText: [countryOption.name, label].join(" "),
    };
  });
  const disciplineOptions = DISCIPLINES.map((discipline) => {
    const label = localizeSearchOptionLabel(discipline.label, discipline.label, locale);
    return {
      value: discipline.label,
      label,
      searchText: [discipline.label, label].join(" "),
    };
  });
