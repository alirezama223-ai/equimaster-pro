        ...
        <div className="max-w-7xl mx-auto px-6 space-y-10">
          ...
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-[#111827]">
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <Detail label={t("profile.breed")} value={horse.breed ?? "—"} />
                <Detail label={t("profile.color")} value={horse.color ?? "—"} />
                <Detail label={t("profile.registration")} value={horse.registrationNumber ?? "—"} />
                <Detail label={t("profile.sire")} value={sireName ?? "—"} />
                <Detail label={t("profile.dam")} value={damName ?? "—"} />
              </div>
          ...
          {tree ? (
            <div className="rounded-3xl border border-white/10 bg-[#08111F] p-4 sm:p-6">
              <h2 className="text-3xl font-bold mb-6">{t("profile.pedigreeTree")}</h2>
          ...
          {traitResult.canManage ? (
            <div className="flex flex-wrap gap-4">
              ...
              >
                {t("profile.manageTraitEvidence")}
              </Link>
              <Link
                href={`/pedigree/${horse.id}/timeline`}
                className="inline-flex rounded-xl border border-white/10 px-5 py-3 text-sm font-semibold text-gray-200 hover:border-blue-500"
              >
                {t("profile.eventTimeline")}
              </Link>
              <Link
                href={`/pedigree/${horse.id}/health`}
                className="inline-flex rounded-xl border border-white/10 px-5 py-3 text-sm font-semibold text-gray-200 hover:border-blue-500"
              >
                {t("profile.healthWellness")}
              </Link>
              <Link
                href={`/pedigree/${horse.id}/analytics`}
                className="inline-flex rounded-xl border border-white/10 px-5 py-3 text-sm font-semibold text-gray-200 hover:border-blue-500"
              >
                {t("profile.viewTrainingAnalytics")}
              </Link>
            </div>
          ) : null}
        </div>
